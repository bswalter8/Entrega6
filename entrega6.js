...
obtenerCatalogo().then(libros_Items =>{
   libros_Items.forEach((item) => {
     let {id, nombre, precio : valor, autor, categoria} = item;
     let agregar_Libro = {
       id, nombre, valor, valor, autor, categoria, 
       cantidad : 0
     }  
...
const run = async () => {
      ...
      const funcion_Ordenar = (selec_Orden) =>{
        let listado = document.getElementById("libros");               
        let o = ordenar(libros_Seleccionados, selec_Orden.value);///
